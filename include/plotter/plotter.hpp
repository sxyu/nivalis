...
struct PointMarker {
    ...
    static constexpr const char* label_repr(int lab) {
        switch (lab) {
            ...
            case LABEL_Y_INT:
                return "y-intercept\n";
            case LABEL_LOCAL_MIN:
                return "local minimum\n";
            case LABEL_LOCAL_MAX:
                return "local maximum\n";
            case LABEL_INTERSECTION:
                return "intersection\n";
            case LABEL_INFLECTION_PT:
...
class Plotter {
   public:
    ...
    // Construct a Plotter.
    // init_expr: initial expression for first function (which is automatically
    // added)
     ...
     * void line(float ax, float ay, float bx, float by, const color::color&,
     float thickness = 1.0);               draw line (ax, ay) -- (bx, by)
     * void polyline(const std::vector<point>& points, const color::color&,
     float thickness = 1., bool closed, bool fill);  draw polyline/polygon
     (filled or non-filled; if filled set then closed is ignored)
     * void rectangle(float x, float y, float w, float h, bool fill, const
     color::color&);                          draw rectangle (filled or
     non-filled)
     * void triangle(float x1, float y1, float x2, float y2,
                  float x3, float y3, bool fill, const color::color& c);
     * void circle(float x, float y, float r, bool fill, const color::color&);
     draw circle (filled or non-filled)
     * void ellipse(float x, float y, float rx, float ry bool fill, const
     color::color&);                           draw axis-aligned ellipse (filled
     or non-filled)
     * void string(float x, float y, std::string s, const color::color&); draw a
     string
     * Adapater may use internal caching and/or add optional arguments to above
     functions; when require_update is set the cache should be
     * cleared. */
    ...
    void draw_grid(GraphicsAdaptor& graph, const View& view) {
        ...
        float y_ax_label_align_x = 0.f, y_ax_label_align_y = 0.5f;
        float x_ax_label_align_x = 0.5f, x_ax_label_align_y = 0.f;

        // Draw axes
        if (enable_axes) {
            if (view.ymin <= 0 && view.ymax >= 0) {
                sy_min = sy0;
                graph.line(0.f, sy_min, view.swid, sy_min, color::DARK_GRAY,
                           2.);
                ++cnt_visible_axis;
            } else if (view.ymin > 0) {
            ...
            if (view.xmin <= 0 && view.xmax >= 0) {
                ...
                graph.line(sx_min, 0.f, sx_min, view.shigh, color::DARK_GRAY,
                           3.);
                ++cnt_visible_axis;
            } else if (view.xmax < 0) {
                sx_min = view.swid - 5;
        ...
        if (polar_grid) {
            double rstep, rmstep;
            auto xmins = view.xmin * view.xmin, ymins = view.ymin * view.ymin,
                 xmaxs = view.xmax * view.xmax, ymaxs = view.ymax * view.ymax;
            ...
            if (cnt_visible_axis == 2)
                ...
                rmin =
                    std::sqrt(std::min(std::min(xmins + ymins, xmins + ymaxs),
                                       std::min(xmaxs + ymins, xmaxs + ymaxs)));
            }
            // Determine maximum distance from origin
            double rmax =
                std::sqrt(std::max(std::max(xmins + ymins, xmins + ymaxs),
                                   std::max(xmaxs + ymins, xmaxs + ymaxs)));
            // Compute minor/major step sizes
            static const double INITIAL_SCREEN_DIAM = sqrt(
            ...
            std::tie(rstep, rmstep) = util::round125(view_diam / screen_diam *
            ...
            double rmhi = std::floor(rmax / rmstep) * rmstep;
            {
                ...
                while (rmlo <= rmhi) {
                    ...
                    if (enable_grid) {
                        graph.ellipse(sx0, sy0, sxr, syr, false, color::GRAY);
                    }
                    // Draw labels on axes
                    if (enable_axes && rmlo > 0.) {
                        auto num_label = prec4(rmlo);
                        // X-axis
                        graph.string(sx0 + sxr, sy0 + 5, num_label,
                                     color::BLACK, x_ax_label_align_x);
                        graph.string(sx0 - sxr, sy0 + 5, num_label,
                                     color::BLACK, x_ax_label_align_x);
                        // Y-axis
                        graph.string(sx0 - 5, sy0 + syr - 2, num_label,
                                     color::BLACK, 1.f, y_ax_label_align_y);
                        graph.string(sx0 - 5, sy0 - syr - 2, num_label,
                                     color::BLACK, 1.f, y_ax_label_align_y);
                    }
            ...
            if (enable_grid) {
                ...
                for (int i = 0; i < 24; ++i) {
                    ...
                    graph.line(sx0 + uxs * rmin, sy0 + uys * rmin,
                               sx0 + uxs * rmax, sy0 + uys * rmax,
                               color::LIGHT_GRAY);
                    // Draw labels
                    ++idx;
                }
                ...
                // #define _PI_STR "pi"
#define _PI_STR u8"\u03C0"
                static const char* angle_labels[] = {u8"0",
                                                     _PI_STR u8"/6",
                                                     _PI_STR u8"/3",
                                                     _PI_STR u8"/2",
                                                     u8"2" _PI_STR u8"/3",
                ...
                // 0, pi/6, ...
                for (int i = 0;
                     i < sizeof(angle_labels) / sizeof(angle_labels[0]); ++i) {
                    ...
                    graph.string(posx, posy, angle_labels[i], color::GRAY,
                                 ((float)ux + 1.f) / 2.f,
                                 ((float)uy + 1.f) / 2.f);
                    ++idx;
                }
            ...
            std::tie(ystep, ymstep) =
                util::round125((view.ymax - view.ymin) / view.shigh * 1. *
                               SCREEN_HEIGHT / 10.8);
            std::tie(xstep, xmstep) = util::round125(
                (view.xmax - view.xmin) / view.swid * 1. * SCREEN_WIDTH / 18.);
            if (enable_grid) {
                // Minor gridlines
                {
                    ...
                    while (xl <= xr) {
                        float sxi =
                            static_cast<float>(view.swid * (xl - view.xmin) /
                                               (view.xmax - view.xmin));
                ...
                }
                {
                    ...
                    while (yb <= yt) {
                        float syi =
                            static_cast<float>(view.shigh * (view.ymax - yb) /
                                               (view.ymax - view.ymin));
            ...
            }
            {
                ...
                while (xml <= xmr) {
                    float sxi =
                        static_cast<float>(view.swid * (xml - view.xmin) /
                                           (view.xmax - view.xmin));
                    if (enable_grid) {
                ...
                while (ymb <= ymt) {
                    float syi =
                        static_cast<float>(view.shigh * (view.ymax - ymb) /
                                           (view.ymax - view.ymin));
                    if (enable_grid) {
                    ...
                    if (enable_axes && ymb != 0) {
                        graph.string(sx_min + 10. - y_ax_label_align_x * 15.,
                                     syi - 2, prec4(ymb), color::BLACK,
        ...
        if (enable_axes && cnt_visible_axis == 2) {
            graph.string(sx_min - 5, sy_min + 5, "0", color::BLACK, 1.0f, 0.0f);
        }
    }
    template <class GraphicsAdaptor>
    ...
    void draw(GraphicsAdaptor& graph, const View& view) {
        if (view.xmin >= view.xmax) {
            draw(graph, View{view.swid, view.shigh, view.xmin + 1e-9, view.xmin,
                             view.ymax, view.ymin});
            return;
        }
        if (view.ymin >= view.ymax) {
            draw(graph, View{view.swid, view.shigh, view.xmax, view.xmin,
                             view.ymin + 1e-9, view.ymin});
            return;
        }

        std::vector<point> points;
        auto draw_single_obj = [&points, &graph, &view](
                                   const DrawBufferObject& obj,
                                   bool is_curr_func) {
            ...
            for (size_t i = 0; i < points.size(); ++i) {
                ...
                auto& npt = points[i];
                npt[0] = static_cast<float>((pt[0] - view.xmin) * view.swid /
                                            (view.xmax - view.xmin));
                npt[1] = static_cast<float>((view.ymax - pt[1]) * view.shigh /
                                            (view.ymax - view.ymin));
            }
            // Draw the object
            if (obj.type <= DrawBufferObject::FILLED_POLYGON) {
                // Polyline
                graph.polyline(points, obj.c,
                               obj.thickness,  //+ (float)is_curr_func,
                ...
                graph.rectangle(
                    ...
                    is_curr_func && obj.c.a <= .8f
                        ? color::color(obj.c.r, obj.c.g, obj.c.b, obj.c.a + .2f)
                        : obj.c,
                    obj.thickness);
            } else if (obj.type <= DrawBufferObject::FILLED_TRIANGLE) {
                // Triangle
                graph.triangle(points[0][0], points[0][1], points[1][0],
                               points[1][1], points[2][0], points[2][1],
                               obj.type == DrawBufferObject::FILLED_TRIANGLE,
                               obj.c);
                ...
                // String
                graph.string(points[0][0], points[0][1], obj.str, obj.c, 0.5f,
                             0.5f /* Center text */);
            }
        };
        ...
        for (size_t i = pt_markers.size() - 1; ~i; --i) {
            ...
            int sy = static_cast<int>((view.ymax - ptm.y) /
                                      (view.ymax - view.ymin) * view.shigh);
            int sx = static_cast<int>((ptm.x - view.xmin) /
                                      (view.xmax - view.xmin) * view.swid);
            color::color border_col =
                ~ptm.rel_func ? funcs[ptm.rel_func].line_color : color::GRAY;
            color::color fill_col((border_col.r + 1.f) / 2.f,
                                  (border_col.g + 1.f / 2.f),
            ...
            graph.rectangle(sx - r, sy - r, 2 * r + 1, 2 * r + 1, true,
                            fill_col);
            graph.rectangle(sx - r, sy - r, 2 * r + 1, 2 * r + 1, false,
                            border_col);
        }
    }
    ...
    std::istream& import_json(std::istream& is,
                              std::string* error_msg = nullptr);

    // Binary serialization, only functions, view, and env (used to sync data to
    // worker before render)
    std::ostream& export_binary_func_and_env(std::ostream& os) const;
    std::istream& import_binary_func_and_env(std::istream& is);

    // Binary serialization, only draw_buf, pt_markers (used to sync data from
    // worker after render)
   ...
   private:
    ...
    void plot_complex(size_t funcid);
   ...
   public:
    ...
    // Set curr_func to this to indicate no current function (i.e. select
    // background)
    ...
    bool polar_grid =
        false;  // If true, draws polar grid and polar (i.e. unsigned) axes
                // labels note this has an effect even when enable_grid = false
    ...
    } passive_marker_click_behavior = PASSIVE_MARKER_CLICK_DRAG_VIEW;

    // Intersection/crit point finding
    // Number of functions above which to stop looking for function
    // intersections and crit points EXCEPT for critical points of curr_func and
    // intersections of curr_func with other functions
    size_t max_functions_find_all_crit_points = 5;

    // Number of functions above which to stop looking for intersections and
    // critical points also disables asymptote detection (quality may worsen)
    ...
    bool loss_detail =
        false;  // Whether some detail is lost (if set, will show error)
   private:
    ...
    uint32_t x_var, y_var, z_var, t_var;  // x,y,z,t variable addresses
