...
constexpr bool is_numeric(char c) { return (c >= '0' && c <= '9') || c == '.'; }
...
constexpr bool is_logic_operator(char c) { return c == '&' || c == '|'; }
...
constexpr bool is_operator(char c) {
    return is_comp_operator(c) || is_logic_operator(c) || is_arith_operator(c);
}
...
size_t find_equality(std::string_view expr, bool allow_ineq = false,
...
void print_complex(std::ostream& os, std::complex<double> n);
