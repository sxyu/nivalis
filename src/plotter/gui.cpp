...
bool is_var_name_reserved(const std::string& var_name) {
    return var_name == "x" || var_name == "y" || var_name == "z";
}
...
bool Plotter::View::operator==(const View& other) const {
    return shigh == other.shigh && swid == other.swid && xmin == other.xmin &&
           xmax == other.xmax && ymin == other.ymin && ymax == other.ymax;
}
...
void Plotter::reparse_expr(size_t idx) {
    ...
    auto& func = funcs[idx];
    func_error.clear();
    func.expr = parse(func.expr_str, env, true, true, 0, &func_error);
...
void Plotter::set_curr_func(size_t func_id) {
    if (func_id != curr_func) func_error.clear();
    if (curr_func < funcs.size()) reparse_expr(curr_func);
    ...
    if (curr_func == funcs.size()) {
        // New function
        Function f;
        f.type = Function::FUNC_TYPE_COMPLEX;
...
void Plotter::add_func() { set_curr_func(funcs.size()); }
...
void Plotter::update_slider_var(size_t idx) {
    ...
    std::string vn = sl.var_name;
    sl.is_imag = util::ends_with(vn, "_im");
    if (sl.is_imag) {
        vn = vn.substr(0, vn.size() - 3);
    }
    ...
    if (vn == "") {
        ...
        // Already has slider
        slider_error = "Duplicate slider for " + vn + "\n";
        sl.var_addr = -1;
        sl.var_name.clear();
    } else if (is_var_name_reserved(vn)) {
        // Not allowed to set in slider (reserved)
        slider_error = vn + " is reserved\n";
        sl.var_addr = -1;
        vn.clear();
    } else {
        sl.var_addr = env.addr_of(vn, false);
        sliders_vars.insert(sl.var_name);
        slider_error.clear();
        copy_slider_value_to_env(idx);
        for (size_t t = 0; t < funcs.size(); ++t) reparse_expr(t);
        require_update = true;
...
void Plotter::add_slider() {
    ...
    while (var_name[0] < 'z' && sliders_vars.count(var_name)) {
...
void Plotter::copy_slider_value_to_env(size_t idx) {
    ...
    if (~sl.var_addr) {
        if (sl.is_imag) {
            env.vars[sl.var_addr].imag(sl.val);
        } else {
            env.vars[sl.var_addr].real(sl.val);
        }
        require_update = true;
    }
}
...
void Plotter::end_slider_animation(size_t idx) {
    if (sliders[idx].animation_dir) {
        ...
        auto it =
            std::find(animating_sliders.begin(), animating_sliders.end(), idx);
...
void Plotter::slider_animation_step() {
    ...
    long delta_t = std::chrono::duration_cast<std::chrono::nanoseconds>(
                       slider_animation_time - slider_animation_prev_time)
                       .count();
    ...
    for (size_t idx : animating_sliders) {
        sliders[idx].val += (double)sliders[idx].animation_dir *
...
void Plotter::reset_view() {
    double wid =
        10. * view.swid / view.shigh * (SCREEN_HEIGHT * 1. / SCREEN_WIDTH);
    view.xmax = wid;
    view.xmin = -wid;
    view.ymax = 6.0;
    view.ymin = -6.0;
...
void Plotter::handle_key(int key, bool ctrl, bool shift, bool alt) {
    ...
    switch (key) {
        ...
        case 173:
            // Zooming +-
            {
                auto fa = 1 + 0.013 * ((key == 45 || key == 189 || key == 173)
                                           ? scale
                                           : -scale);
...
void Plotter::handle_mouse_down(int px, int py) {
    if (!drag_view && !drag_trace) {
        if (px >= 0 && py >= 0 && py * view.swid + px < grid.size() &&
...
void Plotter::handle_mouse_move(int px, int py) {
    if (~drag_marker && drag_marker < pt_markers.size() &&
        ...
        int sx = (int)(std::min(std::max(_X_TO_SX(ptm.x), 0.f),
                                (float)view.swid - 1.f) +
                       0.5f);
        int sy = (int)(std::min(std::max(_Y_TO_SY(ptm.y), 0.f),
                                (float)view.shigh - 1.f) +
                       0.5f);
        if (~grid[sy * view.swid + sx])
            detect_marker_click(sx, sy, true, false);
        require_update = true;
        return;
    }
    if (drag_view) {
        ...
        view.ymin += fy;
        require_update = true;
    } else if (px >= 0 && py >= 0 && py * view.swid + px < grid.size() &&
               ~grid[py * view.swid + px]) {
        // Trace drag mode
...
std::ostream& Plotter::export_json(std::ostream& os, bool pretty) const {
    std::vector<json> jshell, jfuncs, jsliders;
    {
        ...
        for (size_t i = 0; i < env.vars.size(); ++i) {
            ...
            if (std::isnan(env.vars[i].real()) ||
                std::isinf(env.vars[i].real()))
                continue;
    ...
    for (size_t i = 0; i < env.funcs.size(); ++i) {
        auto& f = env.funcs[i];
        if (f.name.size() > 1 && f.name[0] == 'f' && f.n_args == 1) {
    ...
    for (auto& slider : sliders) {
        jsliders.push_back(json{{"var", slider.var_name},
                                {"min", slider.lo},
                                {"max", slider.hi},
                                {"val", slider.val}});
    }
...
std::istream& Plotter::import_json(std::istream& is, std::string* error_msg) {
    ...
    try {
        ...
        if (j.count("view")) {
            ...
            if (jview.is_object()) {
                if (jview.count("xmin"))
                    view.xmin = jview["xmin"].get<double>();
                if (jview.count("xmax"))
                    view.xmax = jview["xmax"].get<double>();
                if (jview.count("xmin"))
                    view.ymin = jview["ymin"].get<double>();
                if (jview.count("xmax"))
                    view.ymax = jview["ymax"].get<double>();
                int old_swid = view.swid, old_shigh = view.shigh;
                if (jview.count("width"))
                    view.swid = jview["width"].get<double>();
                if (jview.count("height"))
                    view.shigh = jview["height"].get<double>();
                // Will resize bounds to fit screen
                // else will become too distorted
                resize(old_swid, old_shigh);
                if (jview.count("axes"))
                    enable_axes = jview["axes"].get<bool>();
                if (jview.count("grid"))
                    enable_grid = jview["grid"].get<bool>();
                if (jview.count("polar"))
                    polar_grid = jview["polar"].get<bool>();
            }
        }
        ...
        if (j.count("sliders") && j["sliders"].is_array()) {
            for (auto& slider : j["sliders"]) {
                ...
                if (slider.count("var")) {
                    ...
                    if (!slider.count("val")) {
                        const std::string& vn = sliders[idx].var_name;
                        if (sliders[idx].is_imag) {
                            sliders[idx].val =
                                env.get(vn.substr(0, vn.size() - 3)).imag();
                        } else {
                            sliders[idx].val = env.get(vn).real();
                        }
                    }
                }
            }
        ...
        if (j.count("funcs") && j["funcs"].is_array()) {
            ...
            for (auto& jfunc : j["funcs"]) {
                ...
                if (jfunc.is_object()) {
                    ...
                    if (jfunc.count("color")) {
                        ...
                        if (jcol.is_string()) {
                            f.line_color =
                                color::from_hex(jcol.get<std::string>());
                        } else if (jcol.is_number_integer()) {
                            f.line_color =
                                color::from_int((size_t)jcol.get<int>());
                        }
                    } else {
                    ...
                    if (jfunc.count("id")) {
                        ...
                        next_func_name =
                            std::max(next_func_name, (size_t)id + 1);
            ...
            funcs.resize(1);
            funcs[0].expr_str = "";
            funcs[0].type = Function::FUNC_TYPE_COMPLEX;
        ...
        if (j.count("internal")) {
            ...
            if (jint.is_object()) {
                ...
                if (jint.count("next_color"))
                    last_expr_color = jint["next_color"].get<double>();
                if (jint.count("color_queue") &&
                    jint["color_queue"].is_array()) {
                    for (auto& jcol : jint["color_queue"]) {
                        reuse_colors.push_back(
                            color::from_hex(jcol.get<std::string>()));
                    }
                }
            }
        ...
        // If one save uses Nivalis expression format and the other uses LaTeX,
        // try to convert
        if (imp_use_latex != use_latex) {
            if (use_latex) {
                for (size_t i = 0; i < funcs.size(); ++i) {
                    funcs[i].expr_str =
                        nivalis_to_latex_safe(funcs[i].expr_str);
                }
            } else {
                for (size_t i = 0; i < funcs.size(); ++i) {
